...
//url을 통해 전달되는 데이터에 한글, 공백 등과 같은 문자가 포함될 경우 제대로 인삭되지 않는 문제 해결
...
app.get('/quiz/q1', (req, res) => {
    fs.readFile(__dirname + '/quiz1.html', 'utf8', (err, text) => {
        res.send(text);
    })
});

app.get('/quiz/q2', (req, res) => {
    fs.readFile(__dirname + '/quiz2.html', 'utf8', (err, text) => {
        res.send(text);
    });
});

app.get('/quiz/q3', (req, res) => {
    fs.readFile(__dirname + '/quiz3.html', 'utf8', (err, text) => {
        res.send(text);
    });
});

app.get('/quiz/q4', (req, res) => {
    fs.readFile(__dirname + '/quiz4.html', 'utf8', (err, text) => {
        res.send(text);
    });
});

app.get('/quiz/q5', (req, res) => {
    fs.readFile(__dirname + '/quiz5.html', 'utf8', (err, text) => {
        res.send(text);
    });
});
...
app.post('/quiz/q1', (req, res) => {
    fs.readFile(__dirname + '/quiz1.html', 'utf8', (err, text) => {
        const score1 = String(req.body.score1);
        ...
        .then(async function(data){//성공했을때 then을 실행
            ...
            const sessionid = req.session.successId;//session의 아이디 값 불러옴
            const idx = user.id.indexOf(sessionid);//session아이디값의 인덱스 번호
            user.score1.splice(idx, 1, score1);//score1의 값을 바꿔줌
            const newdata =  user;
            response.success = true;
...
app.post('/quiz/q2', (req, res) => {
    fs.readFile(__dirname + '/quiz2.html', 'utf8', (err, text) => {
        const score2 = String(req.body.score2);
        ...
        .then(async function(data){//성공했을때 then을 실행
            ...
            const sessionid = req.session.successId;//session의 아이디 값 불러옴
            const idx = user.id.indexOf(sessionid);//session아이디값의 인덱스 번호
            user.score2.splice(idx, 1, score2);//score2의 값을 바꿔줌
            const newdata =  user;
            response.success = true;
...
app.post('/quiz/q3', (req, res) => {
    fs.readFile(__dirname + '/quiz3.html', 'utf8', (err, text) => {
        const score3 = String(req.body.score3);
        ...
        .then(async function(data){//성공했을때 then을 실행
            ...
            const sessionid = req.session.successId;//session의 아이디 값 불러옴
            const idx = user.id.indexOf(sessionid);//session아이디값의 인덱스 번호
            user.score3.splice(idx, 1, score3);//score3의 값을 바꿔줌
            const newdata =  user;
            response.success = true;
...
app.post('/quiz/q4', (req, res) => {
    fs.readFile(__dirname + '/quiz4.html', 'utf8', (err, text) => {
        const score4 = String(req.body.score4);
        ...
        .then(async function(data){//성공했을때 then을 실행
            ...
            const sessionid = req.session.successId;//session의 아이디 값 불러옴
            const idx = user.id.indexOf(sessionid);//session아이디값의 인덱스 번호
            user.score4.splice(idx, 1, score4);//score4의 값을 바꿔줌
            const newdata =  user;
            response.success = true;
...
app.post('/quiz/q5', (req, res) => {
    fs.readFile(__dirname + '/quiz5.html', 'utf8', (err, text) => {
        const score5 = String(req.body.score5);
        ...
        .then(async function(data){//성공했을때 then을 실행
            ...
            const sessionid = req.session.successId;//session의 아이디 값 불러옴
            const idx = user.id.indexOf(sessionid);//session아이디값의 인덱스 번호
            user.score5.splice(idx, 1, score5);//score5의 값을 바꿔줌
            const newdata =  user;
            response.success = true;
